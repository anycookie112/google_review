...
import { updateStore, type StoreUpsertInput } from "@/lib/db/stores";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const updated = await updateStore(currentSlug, input);
    if (!updated) {
      return redirectToPath(buildPathWithQuery("/admin/stores", { error: "not_found" }));
    }

    return redirectToPath(buildPathWithQuery("/admin/stores", { message: "updated" }));
  } catch (error) {
